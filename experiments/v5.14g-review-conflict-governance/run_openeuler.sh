...
EXP_ID="${AGENTLITE_V514G_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V514G_RUN_ROOT:-runs/v5.14g-review-conflict-governance/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V514G_EXPORT_BASE:-exports/v5.14g-review-conflict-governance-$EXP_ID}"

if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "错误：存在已跟踪但未提交的修改，拒绝开始 v5.14g 验收。" >&2
...
for path in "$RUN_ROOT" "$EXPORT_BASE.tar.gz" "$EXPORT_BASE.tar.gz.sha256"; do
  if [[ -e "$path" ]]; then
    ...
    echo "请设置新的 AGENTLITE_V514G_EXP_ID 后重试。" >&2
    exit 2
  fi
done
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.14g-review-conflict-governance
git diff --check
