...
EXP_ID="${AGENTLITE_V514K_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V514K_RUN_ROOT:-runs/v5.14k-typed-reliability-acceptance/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V514K_EXPORT_BASE:-exports/v5.14k-typed-reliability-acceptance-$EXP_ID}"

if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "Error: tracked source changes are present; acceptance is refused." >&2
  echo "Commit, restore, or save those tracked changes before retrying." >&2
  exit 2
fi
...
for path in "$RUN_ROOT" "$EXPORT_BASE.tar.gz" "$EXPORT_BASE.tar.gz.sha256"; do
  if [[ -e "$path" ]]; then
    ...
    echo "Set a new AGENTLITE_V514K_EXP_ID before retrying." >&2
    exit 2
  fi
done
...
echo "[2/5] Generate the v5.14k mechanism acceptance report"
set +e
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.14k-typed-reliability-acceptance
git diff --check
