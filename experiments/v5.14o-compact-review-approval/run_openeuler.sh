...
EXP_ID="${AGENTLITE_V514O_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V514O_RUN_ROOT:-runs/v5.14o-compact-review-approval/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V514O_EXPORT_BASE:-exports/v5.14o-compact-review-approval-$EXP_ID}"
...
for path in "$RUN_ROOT" "$EXPORT_BASE.tar.gz" "$EXPORT_BASE.tar.gz.sha256"; do
  if [[ -e "$path" ]]; then
    ...
    echo "Set a new AGENTLITE_V514O_EXP_ID before retrying." >&2
    exit 2
  fi
done
...
echo "[2/5] Generate the v5.14o mechanism acceptance report"
set +e
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.14o-compact-review-approval
git diff --check
