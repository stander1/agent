...
EXP_ID="${AGENTLITE_V515T_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V515T_RUN_ROOT:-runs/v5.15t-open-attribution-isolation/$EXP_ID}"
TRACE_ROOT="${AGENTLITE_V515T_TRACE_ROOT:-.agentlite-exp/v5.15t-open-attribution-isolation/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V515T_EXPORT_BASE:-exports/v5.15t-open-attribution-isolation-$EXP_ID}"
SCENARIO_FILE="${AGENTLITE_V515T_SCENARIO_FILE:-}"
TEAM_FILE="${AGENTLITE_V515T_TEAM_FILE:-}"
MEMORY_SCOPE="v515t-$EXP_ID"
...
if [[ -z "$SCENARIO_FILE" || ! -f "$SCENARIO_FILE" ]]; then
  echo "Error: AGENTLITE_V515T_SCENARIO_FILE must name an external JSON file." >&2
  exit 2
fi
if [[ -z "$TEAM_FILE" || ! -f "$TEAM_FILE" ]]; then
  echo "Error: AGENTLITE_V515T_TEAM_FILE must name an external JSON file." >&2
  exit 2
fi
...
echo "[1/6] Run open-attribution isolation regressions"
python -m unittest \
...
echo "[4/6] Verify open attribution, cross-source binding, cost, and delivery"
set +e
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.15t-open-attribution-isolation
git diff --check
