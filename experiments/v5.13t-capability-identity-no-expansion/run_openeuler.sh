...
V513T_TEMPERATURE="${AGENTLITE_V513T_TEMPERATURE:-0}"
V513T_MAX_TURNS="${AGENTLITE_V513T_MAX_TURNS:-8}"
...
EXP_ID="${AGENTLITE_V513T_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V513T_RUN_ROOT:-runs/v5.13t-capability-identity/$EXP_ID}"
TRACE_ROOT="${AGENTLITE_V513T_TRACE_ROOT:-.agentlite-exp/v5.13t-capability-identity/$EXP_ID}"
EXPORT_BASE="exports/v5.13t-capability-identity-$EXP_ID"

for path in "$RUN_ROOT" "$TRACE_ROOT" "$EXPORT_BASE.tar.gz"; do
  if [[ -e "$path" ]]; then
    echo "错误：实验路径已经存在，拒绝覆盖证据：$path" >&2
    echo "请设置新的 AGENTLITE_V513T_EXP_ID 后重试。" >&2
    exit 2
  fi
done
...
python "$APP" \
  ...
  --temperature "$V513T_TEMPERATURE" \
  --max-turns "$V513T_MAX_TURNS" \
...
export AGENTLITE_MEMORY_SCOPE="v513t-capability-$EXP_ID"
agentlite autogen \
...
echo "[6/7] 验证逻辑身份、事件去重、上下文不膨胀与交付质量"
set +e
python "$VERIFY" \
  --native-dir "$RUN_ROOT/native" \
  --observed-dir "$RUN_ROOT/observed" \
