...
EXP_ID="${AGENTLITE_V514I_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V514I_RUN_ROOT:-runs/v5.14i-semantic-fidelity-conflict-identity/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V514I_EXPORT_BASE:-exports/v5.14i-semantic-fidelity-conflict-identity-$EXP_ID}"

if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "错误：存在已跟踪但未提交的修改，拒绝开始 v5.14i 验收。" >&2
...
for path in "$RUN_ROOT" "$EXPORT_BASE.tar.gz" "$EXPORT_BASE.tar.gz.sha256"; do
  if [[ -e "$path" ]]; then
    ...
    echo "请设置新的 AGENTLITE_V514I_EXP_ID 后重试。" >&2
    exit 2
  fi
done
...
python -m unittest \
  ...
  tests.test_review_conflict_guard \
  tests.test_v514a_real_memory_fault_injection \
  > "$RUN_ROOT/unittest.txt" 2>&1

echo "[2/5] 生成 v5.14i 机制验收报告"
set +e
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.14i-semantic-fidelity-conflict-identity
git diff --check
