...
EXP_ID="${AGENTLITE_V515X_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V515X_RUN_ROOT:-runs/v5.15x-release-candidate-freeze/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V515X_EXPORT_BASE:-exports/v5.15x-release-candidate-freeze-$EXP_ID}"
...
echo "[1/6] Run release candidate contract regressions"
python -m unittest \
...
echo "[4/6] Build and inspect release candidate artifacts"
set +e
...
echo "[5/6] Verify release candidate identity, contents, and hashes"
set +e
...
python -m compileall -q \
  ...
  examples \
  tests \
  experiments/v5.15x-release-candidate-freeze
git diff --check
