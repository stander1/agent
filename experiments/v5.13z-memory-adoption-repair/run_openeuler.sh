...
EXP_ID="${AGENTLITE_V513Z_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V513Z_RUN_ROOT:-runs/v5.13z-memory-adoption-repair/$EXP_ID}"
EXPORT_BASE="exports/v5.13z-memory-adoption-repair-$EXP_ID"

for path in "$RUN_ROOT" "$EXPORT_BASE.tar.gz"; do
  if [[ -e "$path" ]]; then
    echo "ERROR: acceptance path already exists: $path" >&2
    echo "Set a new AGENTLITE_V513Z_EXP_ID and retry." >&2
    exit 2
  fi
done
...
echo "[1/4] Memory-adoption repair deterministic acceptance"
python "$SCRIPT_DIR/verify_repair_guard.py" \
  --output-dir "$RUN_ROOT/repair-guard"
...
python -m compileall -q agent_runtime experiments/v5.13z-memory-adoption-repair
...
echo "Report: $RUN_ROOT/repair-guard/acceptance_report.md"
