...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Verify the v5.14n Provider regression after the reviewer "
            "artifact-continuity fix."
        )
    )
...
def evaluate(
    *,
    ...
    checks.append(
        _check(
            "formal_run_descends_from_v514m_release",
    ...
    expected_verifier_hash = str(
        lineage.get("v514m_acceptance_verifier_sha256") or ""
    ).lower()
    actual_verifier_hash = _sha256(V514M_DIR / "verify_acceptance.py")
    checks.append(
        _check(
            "v514m_acceptance_verifier_is_bound",
    ...
    report.update(
        {
            "schema_version": (
                "agentlite.v514n.reviewer-artifact-formal-report.v1"
            ),
            "summary": {
                **dict(report.get("summary") or {}),
                "passed": passed,
                "formal_regression_passed": passed,
                "reviewer_artifact_formal_passed": passed,
            ...
            "superseded_checks": [
                {
                    ...
                    "reason": (
                        "v5.14n intentionally preregisters the v5.14m "
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## 场景证据",
        "",
        "| 场景 | 任务 | 引用晋升 | Reviewer 替代 | 所有权错误 | 审批正文泄漏 | 决策记忆 |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    lines.extend(
        [
            ...
            "## 解释边界",
            "",
            "- 三组使用同一份 v5.14m Agent 配置、任务、模型、温度和轮次。",
            "- Reviewer 只负责否决或按引用批准，最终正文仍属于业务成果 Agent。",
            "- Provider 和端到端协作成本、双盲质量及完整交付沿用原正式门禁。",
