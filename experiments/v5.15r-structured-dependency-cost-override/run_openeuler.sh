...
EXP_ID="${AGENTLITE_V515R_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V515R_RUN_ROOT:-runs/v5.15r-structured-dependency-cost-override/$EXP_ID}"
TRACE_ROOT="${AGENTLITE_V515R_TRACE_ROOT:-.agentlite-exp/v5.15r-structured-dependency-cost-override/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V515R_EXPORT_BASE:-exports/v5.15r-structured-dependency-cost-override-$EXP_ID}"
SCENARIO_FILE="${AGENTLITE_V515R_SCENARIO_FILE:-}"
TEAM_FILE="${AGENTLITE_V515R_TEAM_FILE:-}"
MEMORY_SCOPE="v515r-$EXP_ID"
...
if [[ -z "$SCENARIO_FILE" || ! -f "$SCENARIO_FILE" ]]; then
  echo "Error: AGENTLITE_V515R_SCENARIO_FILE must name an external JSON file." >&2
  exit 2
fi
if [[ -z "$TEAM_FILE" || ! -f "$TEAM_FILE" ]]; then
  echo "Error: AGENTLITE_V515R_TEAM_FILE must name an external JSON file." >&2
  exit 2
fi
...
echo "[1/6] Run structured dependency and cost override regressions"
python -m unittest \
  ...
  tests.test_v515p_closed_revision_measurement_acceptance \
  tests.test_v515q_explicit_typed_context_retention \
  tests.test_v515r_structured_dependency_cost_override_acceptance \
...
TRACE_FILE="$(find "$TRACE_ROOT/sessions" -path '*/autogen_driver/trace.jsonl' -type f -print -quit)"
if [[ -z "$TRACE_FILE" || ! -f "$TRACE_FILE" ]]; then
  echo "Error: AutoGen trace is missing under $TRACE_ROOT/sessions." >&2
  exit 2
fi

echo "[4/6] Verify structured dependency, cost override, typed context, and delivery"
set +e
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.15r-structured-dependency-cost-override
git diff --check
