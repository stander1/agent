...
EXP_ID="${AGENTLITE_V515U_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V515U_RUN_ROOT:-runs/v5.15u-semantic-output-boundary/$EXP_ID}"
TRACE_ROOT="${AGENTLITE_V515U_TRACE_ROOT:-.agentlite-exp/v5.15u-semantic-output-boundary/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V515U_EXPORT_BASE:-exports/v5.15u-semantic-output-boundary-$EXP_ID}"
SCENARIO_FILE="${AGENTLITE_V515U_SCENARIO_FILE:-}"
TEAM_FILE="${AGENTLITE_V515U_TEAM_FILE:-}"
MEMORY_SCOPE="v515u-$EXP_ID"
...
if [[ -z "$SCENARIO_FILE" || ! -f "$SCENARIO_FILE" ]]; then
  echo "Error: AGENTLITE_V515U_SCENARIO_FILE must name an external JSON file." >&2
  exit 2
fi
if [[ -z "$TEAM_FILE" || ! -f "$TEAM_FILE" ]]; then
  echo "Error: AGENTLITE_V515U_TEAM_FILE must name an external JSON file." >&2
  exit 2
fi
...
echo "[1/6] Run semantic-output boundary regressions"
python -m unittest \
  ...
  tests.test_v515u_semantic_output_boundary_acceptance \
...
echo "[4/6] Verify semantic output boundary, open attribution, cost, and delivery"
set +e
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.15u-semantic-output-boundary
git diff --check
