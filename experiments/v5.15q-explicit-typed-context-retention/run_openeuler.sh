...
EXP_ID="${AGENTLITE_V515Q_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V515Q_RUN_ROOT:-runs/v5.15q-explicit-typed-context-retention/$EXP_ID}"
TRACE_ROOT="${AGENTLITE_V515Q_TRACE_ROOT:-.agentlite-exp/v5.15q-explicit-typed-context-retention/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V515Q_EXPORT_BASE:-exports/v5.15q-explicit-typed-context-retention-$EXP_ID}"
SCENARIO_FILE="${AGENTLITE_V515Q_SCENARIO_FILE:-}"
TEAM_FILE="${AGENTLITE_V515Q_TEAM_FILE:-}"
MEMORY_SCOPE="v515q-$EXP_ID"
...
if [[ -z "$SCENARIO_FILE" || ! -f "$SCENARIO_FILE" ]]; then
  echo "Error: AGENTLITE_V515Q_SCENARIO_FILE must name an external JSON file." >&2
  exit 2
fi
if [[ -z "$TEAM_FILE" || ! -f "$TEAM_FILE" ]]; then
  echo "Error: AGENTLITE_V515Q_TEAM_FILE must name an external JSON file." >&2
  exit 2
fi
...
echo "[1/6] Run typed context retention regressions"
python -m unittest \
...
echo "[4/6] Verify typed revision, model-visible context, and field-fetch cost"
set +e
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.15q-explicit-typed-context-retention
git diff --check
