...
EXP_ID="${AGENTLITE_V514M_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V514M_RUN_ROOT:-runs/v5.14m-reviewer-artifact-continuity/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V514M_EXPORT_BASE:-exports/v5.14m-reviewer-artifact-continuity-$EXP_ID}"
...
for path in "$RUN_ROOT" "$EXPORT_BASE.tar.gz" "$EXPORT_BASE.tar.gz.sha256"; do
  if [[ -e "$path" ]]; then
    ...
    echo "Set a new AGENTLITE_V514M_EXP_ID before retrying." >&2
    exit 2
  fi
done
...
echo "[2/5] Generate the v5.14m mechanism acceptance report"
set +e
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.14m-reviewer-artifact-continuity
git diff --check
...
git diff HEAD^ HEAD -- \
  ...
  tests \
  experiments/v5.14m-reviewer-artifact-continuity \
  experiments/ordinary-developer-autogen \
