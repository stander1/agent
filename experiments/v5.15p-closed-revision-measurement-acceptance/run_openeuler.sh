...
EXP_ID="${AGENTLITE_V515P_EXP_ID:-$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${AGENTLITE_V515P_RUN_ROOT:-runs/v5.15p-closed-revision-measurement-acceptance/$EXP_ID}"
TRACE_ROOT="${AGENTLITE_V515P_TRACE_ROOT:-.agentlite-exp/v5.15p-closed-revision-measurement-acceptance/$EXP_ID}"
EXPORT_BASE="${AGENTLITE_V515P_EXPORT_BASE:-exports/v5.15p-closed-revision-measurement-acceptance-$EXP_ID}"
SCENARIO_FILE="${AGENTLITE_V515P_SCENARIO_FILE:-}"
TEAM_FILE="${AGENTLITE_V515P_TEAM_FILE:-}"
MEMORY_SCOPE="v515p-$EXP_ID"
...
if [[ -z "$SCENARIO_FILE" || ! -f "$SCENARIO_FILE" ]]; then
  echo "Error: AGENTLITE_V515P_SCENARIO_FILE must name an external JSON file." >&2
  exit 2
fi
if [[ -z "$TEAM_FILE" || ! -f "$TEAM_FILE" ]]; then
  echo "Error: AGENTLITE_V515P_TEAM_FILE must name an external JSON file." >&2
  exit 2
fi
...
python -m compileall -q \
  agent_runtime \
  tests \
  experiments/v5.15p-closed-revision-measurement-acceptance
git diff --check
