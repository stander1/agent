...
正式接管组会启用持久化共享记忆。同一组连续任务必须始终使用相同的 `--data-dir`、工作目录和 Team 成员名称；否则 AgentLite 会将其视为另一个隔离的协作组。Agent 中间输出只进入待审候选池，Team 最终结果通过规则准入后才会成为后续任务可读取的 MemoryView。

观察组的 `--rewrite off` 会强制关闭共享记忆，因此不能用观察组判断跨任务记忆效果。
