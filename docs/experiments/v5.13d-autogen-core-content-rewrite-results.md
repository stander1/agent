# v5.13d AutoGen Core Receiver Hydration Results

## 结论

`v5.13d` 继承 v5.13c 的发送端真实改写和接收端 Prompt View hydration，并在当前 driver phase 下重跑通过。
...
```text
F:\software\anaconda\envs\agentlite-autogen\python.exe .\examples\run_autogen_core_content_rewrite_smoke.py --output-dir .\runs\v5.13d-autogen-core-hydration-20260702-001
```
...
token_delta_native_minus_rewrite: 2172
```
